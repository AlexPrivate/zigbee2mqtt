...
class Controller {
    ...
    publishEntityState(entityID, payload, cache) {
        ...
        if (entity.type === 'device' && appSettings.mqtt.include_device_information) {
            ...
            logger.info(`${messagePayload.device.modelId}`);
        }
            
        if (settings.get().experimental.output === 'json') {
        	this.mqtt.publish(`${settings.get().advanced.base_topic}/${entity.friendlyName}/meta/name`, entity.friendlyName, {retain: true});
	    	for (var property in messagePayload) {
                if(property != 'device'){
                    this.mqtt.publish(`${settings.get().advanced.base_topic}/${entity.friendlyName}/controls/${property}`,`${messagePayload[property]}`, options);
                    this.mqtt.publish(`${settings.get().advanced.base_topic}/${entity.friendlyName}/controls/${property}/meta/type`, 'text', {retain: true});
                }
                else{
                    for (var property2 in messagePayload.device) {
                    this.mqtt.publish(`${settings.get().advanced.base_topic}/${entity.friendlyName}/controls/${property2}`,`${messagePayload.device[property2]}`, options);
                    this.mqtt.publish(`${settings.get().advanced.base_topic}/${entity.friendlyName}/controls/${property2}/meta/type`, 'text', {retain: true});
                    }
                }
			}
