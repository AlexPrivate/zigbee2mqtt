...
const topicRegex = new RegExp(`^${settings.get().advanced.zigbee_topic}/bridge/(bind|unbind)/.+$`);
...
class DeviceBind {
    ...
    onMQTTConnected() {
        this.mqtt.subscribe(`${settings.get().advanced.zigbee_topic}/bridge/bind/+`);
        this.mqtt.subscribe(`${settings.get().advanced.zigbee_topic}/bridge/unbind/+`);
    }
    ...
    parseTopic(topic) {
        ...
        topic = topic.replace(`${settings.get().advanced.zigbee_topic}/bridge/`, '');
