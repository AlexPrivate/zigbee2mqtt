...
const topicRegex = new RegExp(`^${settings.get().advanced.base_topic}/.+/(set|get)$`);
const postfixes = [
...
class DevicePublish {
    ...
    onMQTTConnected() {
        ...
        for (let step = 1; step < maxDepth; step++) {
            const topic = `${settings.get().advanced.base_topic}/${'+/'.repeat(step)}`;
    ...
    parseTopic(topic) {
        ...
        topic = topic.replace(`${settings.get().advanced.base_topic}/`, '');
