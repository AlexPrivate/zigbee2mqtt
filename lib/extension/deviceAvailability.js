...
class DeviceAvailability {
    ...
    publishAvailability(ieeeAddr, available) {
        ...
        const topic = `${settings.get().advanced.base_topic}/${name}/controls/availability`;
        const meta_topic = `${settings.get().advanced.base_topic}/${name}/meta/name`;
        const payload = available ? 'online' : 'offline';
        this.mqtt.publish(meta_topic, name, {retain: true, qos: 0});
        this.mqtt.publish(topic, payload, {retain: true, qos: 0});
        this.mqtt.publish(`${topic}/meta/type`, 'text', {retain: true, qos: 0});
    }
