...
const configRegex = new RegExp(`${settings.get().advanced.config_topic}/\\w+`, 'g');
...
class BridgeConfig {
    ...
    onMQTTConnected() {
        this.mqtt.subscribe(`${settings.get().advanced.config_topic}/+`);
        this.publish();
    }
    ...
    publish() {
        this.mqtt.publish(`${settings.get().advanced.zigbee_topic}/log_level`, logger.transports.console.level, {retain: true, qos: 0}, null);
        this.mqtt.publish(`${settings.get().advanced.zigbee_topic}/permit_join`, this.zigbee.getPermitJoin(), {retain: true, qos: 0}, null);
        this.mqtt.publish(`${settings.get().advanced.zigbee_topic}/log_level/meta/type`, 'text', {retain: true, qos: 0}, null);
        this.mqtt.publish(`${settings.get().advanced.zigbee_topic}/permit_join/meta/type`, 'text', {retain: true, qos: 0}, null);
    }
}
