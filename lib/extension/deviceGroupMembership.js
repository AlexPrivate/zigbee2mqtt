...
const topicRegex = new RegExp(`^${settings.get().advanced.zigbee_topic}/bridge/device/.+/get_group_membership$`);
...
class DeviceGroupMembership {
    ...
    onMQTTConnected() {
        this.mqtt.subscribe(`${settings.get().advanced.zigbee_topic}/bridge/device/+/get_group_membership`);
    }
    ...
    parseTopic(topic) {
        ...
        topic = topic.replace(`${settings.get().advanced.zigbee_topic}/bridge/device/`, '');
